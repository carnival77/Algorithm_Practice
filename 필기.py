...
# 2
리스트의 요소를 순회하면서 수정하는 방식은 문제가 발생할 수 있다. 따라서, 삭제 대상이 아닌 요소를 담은 새 리스트로 교체하는 것이 맞다.
...
list(map(list, zip(*a[::-1]))) # 리맵리의 집은 별 에이의 투닷 -1 에 있어
...
list(map(list, zip(*a)))[::-1] # 반 리맵리의 집은 별 에이 밖 투닷 -1에 있어
